...
- **Le Trou du Cul** (`src/game/trouduc.js`) : jeu de 52 cartes, ordre 3 → 2 (le 2
  est la carte la plus forte), on pose des cartes de même rang en quantité égale
  ou supérieure au pli, sinon on passe ; poser un 8 brûle le pli et permet de
  rejouer immédiatement. Premier à finir = Président, dernier = Trou du Cul.
  *Simplification assumée : pas d'échange de cartes automatique entre le
  Président et le Trou du Cul au début de la manche suivante — seul le
  classement (Président / Vice-Président / Neutre / Trou du Cul) est affiché
  en fin de partie. Facile à ajouter plus tard dans `playAgain` (`engine.js`)
  si vous voulez la règle complète.*
